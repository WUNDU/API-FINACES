...
public class CreditCardController {
    ...
    @Operation(summary = "Associate a credit card to a user", description = "Links a credit card to an authenticated user")
    @ApiResponses({
    ...
    public ResponseEntity<CreditCardResponseDTO> associateCard(
            @Valid @RequestBody AssociateCreditCardRequestDTO dto) throws Exception {
        logger.info("Recebida solicitação para associar cartão para usuário: {}", dto.userId());
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Credit cards retrieved successfully"),
            @ApiResponse(responseCode = "404", description = "User not found")
    })
